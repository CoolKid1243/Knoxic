...
namespace knoxic {
    ...
    KnoxicModel::KnoxicModel(KnoxicDevice &device, const std::vector<Vertex> &vertices) : knoxicDevice(device) {
    ...
    void KnoxicModel::createVertexBuffers(const std::vector<Vertex> &vertices) {
        ...
        void *data;
        vkMapMemory(knoxicDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
        memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
        vkUnmapMemory(knoxicDevice.device(), vertexBufferMemory);
    }
